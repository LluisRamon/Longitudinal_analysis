...
```{r qplot, fig.cap = "PCV evolution for each cow on every dose. Cows are colored in the same fashion for each dose. \\label{fig:qplot}", fig.pos = "H"}
...
```{r boxplot, fig.cap = "Boxplot of PCV values for each time and dose. \\label{fig:boxplot_time_dose}", fig.pos = "H"}
...
print(xtable(missingsTable, digits = 0, caption = "Number of missings in PCV for time and dose"), comment = FALSE, , table.placement = "H")
```
...
print(xtable(cor(cows.w[, c("1", "2", "3")], use = "pairwise.complete.obs"), caption = "TODO(Mathieu). \\label{tab:correlation}"), comment = FALSE, table.placement = "H", include.rownames = FALSE)
```

TODO(Mathieu): Revise text

Correlation structure was calculated for this data between measurements of time. In Table \ref{tab:correlation} it can be shown there is a positive correlation. Possible correlation structures for this kind of data could be an autoregressive model AR(1) or a Compound Symmetry that implies that all distinct members of a cluster are equally correlated with each other. Total variation can be partitioned into the within a cluster component and the unshared component.
...
```{r slope_graph, fig.cap = "TODO(Mathieu) Something about the plot \\label{slope_graph}", fig.pos = "H"}
...
```{r random_graph, fig.cap= "TODO(Mathieu). \\label{random_graph}"}
...
TODO(Mathieu): Explain Further research

We did not explore interaction between time and dose in the linear mixed models.
Imputate missing data.
Try more correlation strctures or try to get the correct one.
Contact researchers for info about assumptions. Explore possiblity to add more covariates.
