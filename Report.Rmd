...
print(xtable(missingsTable, digits = 0, caption = "Number of missings in PCV for time and dose"), comment = FALSE)
```
...
print(xtable(cor(cows.w[, c("1", "2", "3")], use = "pairwise.complete.obs"), caption = "TODO(Mathieu)"), comment = FALSE)
```
...
print(xtable(taula[,-(3:4)], label="anovas:Random_slope_intercept", caption="Anova table comparing the Fixed effects model (no random effects) with the random intercept and the random slope models."), comment = FALSE)
```
...
print(xtable(taula, label="intervals", caption="Summary of the estimations for the parameters for the random slope model adding the number of births."), include.rownames=F, comment = FALSE)
...
```{r slope_graph, fig.cap = "TODO(Mathieu) Something about the plot \\label{slope_graph}"}
...
```{r validacio_nested, echo=FALSE, fig.cap='Analyses of the residuals of the nested model.\\label{nesteddiagnostic}'}
## diagnostic:
par(mfrow=c(2,2), mar=c(4,4,2,2), cex=0.6)
